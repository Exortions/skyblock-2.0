...
public class ZombieSword extends SkyblockItem {

    private static final HashMap<UUID, Integer> charges = new HashMap<>();

    public ZombieSword() {
        super(Skyblock.getPlugin().getItemHandler().getItem("ZOMBIE_SWORD.json"), "zombie_sword");

        new BukkitRunnable() {
            @Override
            public void run() {
                charges.replaceAll((u, v) -> 4);
            }
    ...
    public void onRightClick(PlayerInteractEvent event, HashMap<String, Object> data) {
        if (!charges.containsKey(event.getPlayer().getUniqueId())) charges.put(event.getPlayer().getUniqueId(), 4);
        ...
        for (Player nearbyPlayer : nearby) {
            ...
            nearbySkyblockPlayer.setStat(SkyblockStat.HEALTH, Math.min(nearbySkyblockPlayer.getStat(SkyblockStat.HEALTH) + 40, nearbySkyblockPlayer.getStat(SkyblockStat.MAX_HEALTH)));
        }
